...
import { getDeadStockProducts } from "@/lib/hotSelling";
...
export default async function IntelligencePage({
  ...
  const [nearExpiryItems, deadStock] = await Promise.all([
    db.expiryItem.findMany({
      orderBy: { expiryDate: "asc" },
      take: 100,
    }),
    getDeadStockProducts(session.orgId),
  ]);
  ...
  return (
    <div className="mx-auto max-w-2xl space-y-6">
      ...
      <Card className="overflow-x-auto">
        <div className="mb-1 flex flex-wrap items-center justify-between gap-2">
          <h1 className="text-lg font-bold text-slate-900">Dead Stock / Slow Movers</h1>
          <ExportExcelButton
        ...
        <table className="w-full min-w-[500px] text-left text-sm">
          <thead>
            <tr className="border-b border-slate-200 text-slate-500">
              ...
              <th className="py-2 pr-4">Tied-Up Value</th>
            </tr>
          </thead>
          <tbody>
            {deadStock.map((p) => (
              <tr key={p.id} className="border-b border-slate-100">
                ...
                <td className="py-2 pr-4 font-semibold text-red-700">
                  ₹{p.tiedUpValue.toLocaleString("en-IN")}
                </td>
              </tr>
            ))}
            {deadStock.length === 0 && (
